...
while isempty(stopflag)
  ...
  [TmpBestFitness,TmpBestIndex]=min(fitness.raw);
      ...
      for k=current_kvec
        ...
        % Resample, until fitness is not NaN
          if k <= lambda  % regular samples (not the re-evaluation-samples)
            ...
            if flgDiagonalOnly  
              arx(:,k) = TmpXBest + StdScale * sigma * diagD .* arz(:,k);              % Eq. (1)
            else
              arx(:,k) = TmpXBest + StdScale * sigma * (BD * arz(:,k));                % Eq. (1)
            end
          else % re-evaluation solution with index > lambda
            if flgDiagonalOnly  
              arx(:,k) = arx(:,k-lambda) + StdScale * (noiseEpsilon * sigma) * diagD .* randn(N,1);
            else
              arx(:,k) = arx(:,k-lambda) + StdScale * (noiseEpsilon * sigma) * (BD * randn(N,1));
            end
          end
