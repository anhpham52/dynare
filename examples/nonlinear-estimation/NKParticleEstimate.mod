...
tmp_rand = randi(intmax);
spmd;rng(tmp_rand,'twister');end;
// set the number of particles
options_.particle.number_of_particles = 50;
// use an efficient particle filter based on a Gaussian proposal distribution
options_.particle.filter_algorithm = 'cpf';
   // use an efficient particle filter based on Gaussian mixtures
   // options_.particle.filter_algorithm = 'gmf';
// use smooth resampling, in order to enable the use of standard maximisation algorithms
options_.particle.resampling.method.kitagawa = 0;
options_.particle.resampling.method.stratified = 0;
options_.particle.resampling.method.smooth = 1;
// use monomial cubature for approximating Gaussian integrals
...
options_.particle.initialization = 1;
   // use a (slower) Monte-Carlo approximation
   // options_.particle.initialization = 2;
// allow us to use fmincon, despite the non-smooth objective
